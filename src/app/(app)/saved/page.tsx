...
import { BookmarkCheck } from 'lucide-react'; // Removed LogIn
...
export default function SavedPage() {
  ...
  const bookmarkedSheets = useMemo(() => {
    if (!isAppContextReady) return []; // Simplified: data depends only on AppContext
    return CHEAT_SHEETS_DATA.filter((sheet) => bookmarks.includes(sheet.id));
  }, [bookmarks, isAppContextReady]);
  ...
  // Removed login check for displaying saved sheets
